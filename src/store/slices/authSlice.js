...
export const logout = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      console.warn(
        "âš ï¸ Server logout failed:",
        err.response?.status,
        err.response?.data
      );

      if (err.response?.status === 401) {
        // Session already gone â€” this is fine, proceed with client-side logout
        console.log(
          "âœ… Session already expired, proceeding with client logout"
        );
        return;
      }
      ...
      console.warn(
        "âš ï¸ Server logout failed, but proceeding with client logout for security"
      );
      ...
      localStorage.removeItem("auth_token");
      localStorage.removeItem("lastActivity");
      console.log("âœ… Client-side logout data cleared");
    }
  }
);
